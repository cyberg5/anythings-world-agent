...
export async function buildResearchDossier(candidate: TrendCandidate): Promise<ResearchDossier> {
  // Scripts now run ~80-100s (10-16 segments) — need enough distinct facts
  // to avoid padding/repetition, so we search wider and extract more.
  const hits = await searchWeb(`${candidate.topic} facts documented evidence`, 12);
  ...
  const facts = await askForJson<SourcedFact[]>(
    `You extract only claims that are EXPLICITLY stated in the provided source snippets.
     ...
     extract fewer facts rather than padding. Return ONLY a JSON array of 12-18 objects:
     ...
     needs to hold a viewer's attention for 80-100 seconds, not just list facts.`,
