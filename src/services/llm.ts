...
export async function askForJson<T>(systemPrompt: string, userPrompt: string): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // A free model occasionally returns a completely EMPTY response (seen
      // in production, 3 times in one run) — with temperature 0.7, retrying
      // the identical request has a real chance of getting a non-empty,
      // valid completion next time (unlike Piper's TTS, this isn't
      // deterministic), so treat it the same as a transient network error.
      const isEmptyResponse = /LLM did not return valid JSON. Raw output:\s*$/i.test(message);
      if ((!isTransient && !isEmptyResponse) || attempt === maxAttempts) throw err;
