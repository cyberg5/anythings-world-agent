...
3. **Narrator voice** — free, nothing to sign up for. Uses Microsoft Edge's
   Read Aloud engine via the `msedge-tts` package. Optionally set `TTS_VOICE`
   in `.env` to pick a different voice/language (default: `en-US-AndrewNeural`).
...
- **Free narration**: voice is synthesized via Microsoft Edge's Read Aloud
  engine (the `msedge-tts` package) — no API key, no cost. It's an unofficial
  use of a Microsoft endpoint, so if it ever breaks unexpectedly, check
  npmjs.com/package/msedge-tts for an updated version before assuming your
  own setup is wrong.
